<?php 
 ...
 // session_start();
 $mnu_title = "Type Client";
 $page_title = "Liste des Types Clients";
 $home_page = "index.php";
 $active = "type_client";
 $parambase = " active";
...
jQuery(document).delegate('a.delete-item', 'click', function(e) {
		...
		var label=$('tr.item-row[data-id="' + itemId + '"]').find('span.sn').html();     
 					...
 					swal({
 										...
 										if (isConfirm) {     
 											   var view_mode ="delete_type_client";
 											  $.ajax({
 												   url:"controller.php",
 												   method:"POST",
 					   dataType:"json",
 ...
 <?php //if($utilisateur->HasDroits("12_38")){  ?>	 $("#add_line").click(function () {	
					...
					ClearR();
					 $("#dlg_frm #view").val("create_type_client");
 ...
 jQuery(document).delegate('a.edit-item', 'click', function(e) {
		...
		var itemId = $(this).parents('tr.item-row').attr('data-id'); 
		var label=$('tr.item-row[data-id="' + itemId + '"]').find('span.sn').html();    
		$("#dlg_main #view").val("edit_type_client");
