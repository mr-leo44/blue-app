...
class MarqueCompteur{
  ...
  public $date_synchro;
  public $is_sync;
  private $table_name='t_param_marque_compteur';
  private $connection;
  ...
  function GetDetail(){
   $query = "SELECT * FROM " . $this->table_name . " WHERE code = ? 	LIMIT 0,1";//t_param_marque_compteur
   $stmt = $this->connection->prepare($query);
    $this->code=strip_tags($this->code);
  ...
  function GetIDonLabel($LABEL){
   $query = "SELECT code,libelle FROM " . $this->table_name . " WHERE libelle = ? 	LIMIT 0,1";//t_param_marque_compteur
   $stmt = $this->connection->prepare($query);
   $this->code=strip_tags($id);
  ...
  function GetDetailIN(){
   ...
   $stmt->execute(); 
   $row = $stmt->fetch(PDO::FETCH_ASSOC); 
   $this->libelle=strip_tags($row["libelle"]);
  }
  
  
  
  function read(){ 
   $query = "SELECT code,libelle,annule,n_user_annule,motif_annulation,date_synchro,is_sync FROM " . $this->table_name . " where is_other='0' and annule='0' ORDER BY libelle";
   $stmt = $this->connection->prepare( $query );
   $stmt->execute();
   return $stmt;
  }
