...
//  ALPHABETIQUE COMPTEUR
    public function getUID($generatoras_shop_base, $Lastgen_field, $canbeReset, $originTable, $Pkey, $separator = '') {
        ...
        $stmt->execute();
        $yearLasto = Utils::GetServerDateOrTime("Y");
        ...
        if (!empty($row)) {
			...
			if($this->has_signature == TRUE){
				...
				$query = "insert into " . $generatoras_shop_base . " (annos,datesys,$this->Signature_fld) values ('" . $yearLasto . "','" . Utils::GetServerDateOrTime("DT") . "','" .$this->Signature_Value. "')";
				$stmt = $this->connection->prepare($query);
				$stmt->execute();
				// $stmt->execute(array($shop_id));
				$retoNB = '0';
				...
				//////// initialisation des numeros auto 
				$query = "insert into " . $generatoras_shop_base . " (annos,datesys) values ('" . $yearLasto . "','" . Utils::GetServerDateOrTime("DT") . "')";
				$stmt = $this->connection->prepare($query);
				$stmt->execute();
				// $stmt->execute(array($shop_id));
				$retoNB = '0';
...
//public function uniqUid($len = 13) {  
 /*   function uniqUid($table, $key_fld) {
	...
	function AvoidMaxNested($Pkey, $bytes, $originTable){
        ...
        if ($this->VerifierExistance($Pkey, $bytes, $originTable)) {
            $bytes = $this->GetHigher($Pkey, $originTable);
			// var_dump($bytes);
