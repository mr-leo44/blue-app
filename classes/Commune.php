...
class Commune{
  ...
  public $date_synchro;
  public $is_sync;
  private $table_name='t_param_commune';
  private $connection;
  
  function Create(){
   $query = "INSERT INTO " . $this->table_name . "  SET code=:code,libelle=:libelle,id_province=:id_province,n_user_create=:n_user_create,is_sync=:is_sync";
   ...
   return $result;
    ...
    function GetProvinceCommune($id_){
  ...
  function Modifier(){
   $query = "UPDATE " . $this->table_name . "  SET libelle=:libelle,id_province=:id_province,n_user_update=:n_user_update,date_update=:date_update WHERE code=:code";
   $stmt = $this->connection->prepare($query);
   $this->code=strip_tags($this->code);
   $this->libelle=strip_tags($this->libelle);
   $this->id_province=strip_tags($this->id_province);
  ...
  function read(){ 
   $query = "SELECT code,libelle,id_province FROM " . $this->table_name . " ORDER BY libelle";
   $stmt = $this->connection->prepare( $query );
   $stmt->execute();
   return $stmt;
  }
  function readAll($from_record_num, $records_per_page){ 
   $query = "SELECT code,libelle,id_province FROM " . $this->table_name . " ORDER BY libelle ASC LIMIT {$from_record_num}, {$records_per_page}";
   $stmt = $this->connection->prepare( $query );
   $stmt->execute();
   return $stmt;
  }
  public function search($search_term, $from_record_num, $records_per_page){
   $query = "SELECT code,libelle,id_province  FROM " . $this->table_name  . " WHERE libelle LIKE :search_term  ORDER BY libelle ASC LIMIT :from, :offset";
   $stmt = $this->connection->prepare( $query );
