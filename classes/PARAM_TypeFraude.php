...
class PARAM_TypeFraude
{
   ...
   public $is_sync;
   public $annule;
   private $table_name = 't_param_type_fraude';
   private $connection;
   ...
   function Create()
   {
      ...
      if ($row_avoid != false) {
         $result["error"] = 1;
         if (strtolower($row_avoid["code_label"]) == strtolower($this->code_label)) {
            $result["message"] = "Le code  " . $this->code_label . " existe déjà";
         } else if (strtolower($row_avoid["libelle"]) == strtolower($this->libelle)) {
            $result["message"] = "Il y a déjà un type fraude avec le libellé (" . $this->libelle . ")";
         }
      } else {
