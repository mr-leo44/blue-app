<?php 
 ...
 // session_start();
 $mnu_title = "Codes Diagnostics";
 $page_title = "Liste des Codes des Diagnostics";
 $home_page = "index.php";
 $active = "type_diagnostic";
 $parambase = " active";
...
$utilisateur->readOne();
                            ...
                            <div class="card">
                                ...
                                <div class="form-group">
                                    <div class="table-responsive table-bordered table-hover" style="height:350px;">
                                        <table class="table no-wrap p-table lignes_install ui-sortable" id="lignes_install"><thead><tr><th style="width:95%"><?php echo $mnu_title; ?></th><th><a class="btn btn-xs " id="add_line"><i class="fas fa-plus"></i></a></th> </tr>
                                            </thead>											
                                            <tbody>
											<?php 
										...
										while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
											echo '<tr class="item-row" data-id="' . $row["code"] . '" ><td style="width:95%"><span class="clb">' . $row["code_label"] .'</span> - <span class="sn">' . $row["libelle"] .'</span></td><td><a class="btn btn-xs edit-item"><i class="fas fa-pencil-alt"></i></a><a class="btn btn-xs delete-item"><i class="fas fa-trash"></i></a></td></tr>';
										}
											?>
                                            </tbody>
                                        </table>
 ...
 <div class="modal fade" id="dlg_main" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
   ...
   <form id="dlg_frm" method="post" action="controller_diagnostic.php" enctype="multipart/form-data">  
 		   <input name="code"  id="code" type="hidden"> 
 		   <input name="view" id="view" type="hidden"> 
		<div class="form-group">
                 <label>Code</label>
                 <div class="input-group"  style="width: 100%;" > 
                   <input type="text" class="form-control pull-right" name="code_label" id="code_label" required>
                 </div> 
         </div>
		<div class="form-group">
                 <label>Libellé Code Diagnostic</label>
                 <div class="input-group"  style="width: 100%;" > 
...
jQuery(document).delegate('a.delete-item', 'click', function(e) {
		...
		var code_label=$('tr.item-row[data-id="' + itemId + '"]').find('span.clb').html();     
 					...
 					swal({
										...
										ShowLoader("Suppression en cours...");
 											   var view_mode ="delete_code";
 											  $.ajax({
 												   url:"controller_diagnostic.php",
 												   method:"POST",
 					   dataType:"json",
 ...
 jQuery(document).delegate('a.edit-item', 'click', function(e) {
		...
		var label=$('tr.item-row[data-id="' + itemId + '"]').find('span.sn').html();    
		var code_label=$('tr.item-row[data-id="' + itemId + '"]').find('span.clb').html();    
		$("#dlg_main #view").val("edit_code");
		$('#dlg_title').html('Modification <?php echo $mnu_title; ?>');				 		
		$('#code').val(itemId);		
		$('#libelle').val(label);
		$('#code_label').val(code_label);
		$('#dlg_main').modal('show');
 ...
 $("#srch-term-entity").keyup(function(){
            ...
            var code_label =  row.find('span.clb').html().toString().toLowerCase();
           
 
			if (client_label.indexOf(val) != - 1||code_label.indexOf(val) != - 1)
            {
            row.show();
                    //return false;
            }
