                            ...
                            <div class="card">
                                ...
                                <div class="form-group">
                                    <div class="table-responsive table-bordered table-hover" style="height:350px;">
                                        <table class="table no-wrap p-table lignes_install ui-sortable" id="lignes_install"><thead><tr><th style="width:95%">Quartier</th><th><a class="btn btn-xs " id="add_line"><i class="fas fa-plus"></i></a></th> </tr>
                                            </thead>											
                                            <tbody>
                                            </tbody>
                                        </table>
 ...
 $('.import-csv').click(function() {
   ...
   $('#btn_import_entity').click(function() {
        ...
        var formData = new FormData(form);
         ...
         $("#loading_msg").html("Importation des avenues pour le  <?php echo  $mnu_title; ?> " + Site +" en cours...");
			$("#loadMe").modal({
			  backdrop: "static", 
			  keyboard: false, 
			  show: true 
...
jQuery(document).delegate('a.delete-item', 'click', function(e) {
		...
		var label=$('tr.item-row[data-id="' + itemId + '"]').find('span.sn').html();     
 					...
 					swal({
 										title: "Information",
 										text: "Voulez-vous supprimer le quartier ("+label+")?",
 										type: "warning",
 ...
 <?php //if($utilisateur->HasDroits("12_38")){  ?>	 $("#add_line").click(function () {	
 			ClearR();	 
			var parent_id = $('#commune_id').val()!=null?$('#commune_id').val():'';
			if(parent_id == ''){
				...
				return false;
					 ...
					 $("#dlg_frm #view").val("create_quartier");
 					 $("#dlg_title").html("CREATION QUARTIER");
 					  $("#dlg_main").modal("show");
 	}); <?php //} ?>
 ...
 jQuery(document).delegate('a.edit-item', 'click', function(e) {
		...
		$('#dlg_title').html('MODIFICATION QUARTIER');				 		
		$('#code').val(itemId);		
		$('#libelle').val(label);
		$('#parent_id').val(parent_id);
		$('#dlg_main').modal('show');
