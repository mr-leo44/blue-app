...
// session_start();
$mnu_title = "Statutclient";
$page_title = "Liste des Statutclients";
$home_page = "index.php";
$active = "statutclient";
$parambase = " active";
...
/*
 ...
 include_once "classes/class.statutclient.php";*/
$database = new Database();
$db = $database->getConnection();
$item = new Statutclient($db);
		...
		$(function() {
			...
			<?php if ($utilisateur->HasDroits("12_40")) {  ?> $(".delete").click(function() {
					...
					swal({
						...
						if (isConfirm) {
							var view_mode = "delete_statutclient";
							$.ajax({
								url: "controller.php",
								method: "POST",
								data: {
			...
			<?php if ($utilisateur->HasDroits("12_38")) {  ?> $("#btn_new_").click(function() {
					ClearForm();
					$("#view").val("create_statutclient");
					$("#Heading").html("CREATION STATUTCLIENT");
					$("#dlg_main").modal("show");
				});
			<?php } ?>
			...
			<?php if ($utilisateur->HasDroits("12_39")) {  ?>
				$(".edit").click(function() {
					ClearForm();
					var jeton_actuel = jQuery(this).attr("data-id");
					$("#Heading").html("MODIFICATION INFORMATIONS STATUTCLIENT");
					$.ajax({
						url: "controller.php",
						method: "GET",
						dataType: "json",
						data: {
							view: "detail_statutclient",
							code: jeton_actuel
						},
						success: function(result, statut) {
							$("#view").val("edit_statutclient");
