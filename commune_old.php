...
$utilisateur->readOne();
 ...
 $stmt = null;
 $page_url = "commune.php?";
 
// calculate for the query LIMIT clause
$from_record_num = ($records_per_page * $page) - $records_per_page;

 $category_id = '8';//Commune
 if($search_term == ""){	
 	$stmt = $item->readAll($from_record_num, $records_per_page,$category_id);
 	$total_rows=$item->countAll($category_id); 
 }else{
 $page_url.="s={$search_term}&";
 $stmt = $item->search($search_term, $from_record_num, $records_per_page,$category_id);
 $total_rows=$item->countAll_BySearch($search_term,$category_id);
 }
 ...
 <div class="container-fluid  dashboard-content">	
	 <div class="row">
				  <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
					...
					<a href="<?php echo 'commune.php';?>" class="breadcrumbs_home"><i class='fab fa-fw fa-wpforms nav_icon'></i>  <?php echo $mnu_title; ?></a> <span class="raquo">»</span></div>
                        </div>
                    </div>
                </div>
 ...
 <th style="width:5%;"></th>
                                        ...
                                        <tbody>
         <?php
 				if($utilisateur->HasDroits("10_520"))
 				{ 	 
 								$num_line=0;
...
// }
   ...
   <form id="mainForm" method="post" action="controller.php" enctype="multipart/form-data">  
         ...
         <div class="form-group">
                 ...
                 <div class="input-group"  style="width: 100%;" > 
                   <select  class="form-control select2" name="id_province" id="id_province" style="width: 100%;">
 ...
 <?php //if($utilisateur->HasDroits("12_40")){  ?>	 $(".delete").click(function () {
 ...
 <?php //if($utilisateur->HasDroits("12_38")){  ?>	 $("#btn_new_").click(function () {	
 					 ClearForm();
 					 $("#dlg_main #view").val("create_commune");
 					 $("#dlg_main #Heading").html("CREATION COMMUNE");
 					  $("#dlg_main").modal("show");
 	}); <?php //} ?>
   function ClearForm(){
 	$("#dlg_main #view").val("");
 ...
 <?php //if($utilisateur->HasDroits("12_39")){  ?>
 	$(".edit").click(function () {	
 				 ClearForm();
 				var jeton_actuel = jQuery(this).attr("data-id");
 				$("#dlg_main #Heading").html("MODIFICATION INFORMATIONS COMMUNE");
 				  $.ajax({
 					   url:"controller.php",
 					   method:"GET",
 					   dataType:"json",
 					   data: {view:"detail_commune",code:jeton_actuel},
 					   success : function(result, statut){  
 							$("#dlg_main #view").val("edit_commune");
 							$("#code").val(result.data.code);
 $("#code").val(result.data.code);
 $("#libelle").val(result.data.libelle);
 $("#id_province").val(result.data.parent_id).change();
 							$("#dlg_main").modal("show");
 					   },
 					   error : function(resultat, statut, erreur){
 					   }
