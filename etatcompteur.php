<?php
// session_start();
$mnu_title = "EtatCompteur";
$page_title = "Liste des EtatCompteurs";
$home_page = "index.php";
$active = "etatcompteur";
$parambase = " active";
...
/*
 ...
 include_once "classes/class.etatcompteur.php";
 */

$database = new Database();
$db = $database->getConnection();
$item = new EtatCompteur($db);
						...
						<div class="page-header">
							...
							<div class="page-breadcrumb">
								<nav aria-label="breadcrumb">
									<ol class="breadcrumb">
										<li class="breadcrumb-item"><a href="<?php echo $home_page; ?>" class="breadcrumb-link"><?php echo $APP_NAME; ?></a></li>
										<li class="breadcrumb-item active" aria-current="page"><?php echo $mnu_title; ?></li>
									</ol>
								</nav>
							</div>
						</div>
								...
								<table class="table table-bordered table-hover">
 								...
 								<ul class="dropdown-menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(705px, 40px, 0px); top: 0px; left: 0px; will-change: transform;">';
												...
												if ($utilisateur->HasDroits("12_40")) {
													echo '<a href="#" class="dropdown-item delete" data-name="' . $row_["intutile"] . '" data-id="' . $row_["code"] . '">Supprimer</a>';
												}
										?>
												</ul>
												</td>
					...
					<form id="mainForm" method="post" action="controller.php" enctype="multipart/form-data">
						...
						<div class="form-group">
							...
							<div class="input-group" style="width: 100%;">
								<input type="text" class="form-control pull-right" name="intutile" id="intutile">
							</div>
						</div>
		...
		$(function() {
			...
			<?php if ($utilisateur->HasDroits("12_40")) {  ?> $(".delete").click(function() {
					...
					swal({
						...
						if (isConfirm) {
							var view_mode = "delete_etatcompteur";
							$.ajax({
								url: "controller.php",
								method: "POST",
								data: {
			...
			<?php if ($utilisateur->HasDroits("12_38")) {  ?> $("#btn_new_").click(function() {
					ClearForm();
					$("#view").val("create_etatcompteur");
					$("#Heading").html("CREATION ETATCOMPTEUR");
					$("#dlg_main").modal("show");
				});
			<?php } ?>
			...
			<?php if ($utilisateur->HasDroits("12_39")) {  ?>
				$(".edit").click(function() {
					ClearForm();
					var jeton_actuel = jQuery(this).attr("data-id");
					$("#Heading").html("MODIFICATION INFORMATIONS ETATCOMPTEUR");
					$.ajax({
						url: "controller.php",
						method: "GET",
						dataType: "json",
						data: {
							view: "detail_etatcompteur",
							code: jeton_actuel
						},
						success: function(result, statut) {
							$("#view").val("edit_etatcompteur");
							$("#code").val(result.data.code);
							$("#code").val(result.data.code);
							$("#intutile").val(result.data.intutile);
							$("#dlg_main").modal("show");
						},
						error: function(resultat, statut, erreur) {}
					});
