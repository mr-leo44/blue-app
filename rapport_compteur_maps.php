...
// session_start();
$mnu_title = "Maps";
$page_title = "Maps - Liste des contrôles";
...
$page_c = 'rapport_compteur_maps.php';
...
<body>
                    ...
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                        <div class="card mb-1">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="h5 font-weight-bold text-primary">Maps - Journal des contrôles</div>
                                    ...
                                    <div class="col-sm-3">
                                        <div class="form-group text-left mb-0 mt-1" id='bloc_installateur'>
                                            <select class='form-control select2' style='width: 100%;' id='filtre' name='filtre[]' required multiple="multiple">
                                                ...
                                                $stmt_select = $commune->GetProvinceVille($USER_SITE_PROVINCE);
                                                while ($row_select = $stmt_select->fetch(PDO::FETCH_ASSOC)) {
                                                    echo "<option value=t_param_cvs.code='" . $row_select["code"] . "'>Ville - " . $row_select["libelle"] . "</option>";
                                                }
                        ...
                        <div style="width: 100%!important; height:60vh!important;background-color : white; " id="maps"></div>
                        <div id="paginate"></div>
                    </div>
  ...
  */
    ...
    <!-- prettier-ignore -->
    <!-- <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDi2Iz5YtJ8A49Wo3ckctgCK7GA64xV3Hc&map_ids=3e6e38413e9ecb6d&callback=initMap"></script> -->

    <script src="https://maps.google.com/maps/api/js?key=<?= $_ENV['MAPS_KEY'] ?>&map_ids= <?= $_ENV['MAPS_ID'] ?>&callback=initMap" type="text/javascript"></script>
    ...
    <script type="text/javascript">
        ...
        function displayRecords(numRecords, pageNum, v_mode) {
            ...
            $.ajax({
                type: "GET",
                url: "controller.php",
                data: "view=get_control_list&show=" + numRecords + "&page=" + pageNum + "&Du=" + du + "&Au=" + au + "&s=" + s + "&view_mode=" + v_mode + "&filtre=" + filtre,
                cache: false,
                dataType: "json",
                beforeSend: function() {
                ...
                success: function(result) {
                    ...
                    if (result.count > 0) {
                        ...
                        if (map) {
                            ...
                            // Effacement du tableau markers
                            markers = [];

                            for (item in items) {
                                if (items[item].gps_latitude_control && items[item].gps_longitude_control) {
                                    var position = {
                                        lat: parseFloat(items[item].gps_latitude_control),
                                        lng: parseFloat(items[item].gps_longitude_control)
                                    }
                                    var marker = new google.maps.Marker({
                                        position,
                                        title: items[item].adresse,
                                        map: map
                                    });
                                    const contentString = `
                                    <div class="p-4">
                                        <a class="btn btn-outline-light float-right ml-1 view-fiche" data-id="${items[item].ref_fiche_controle}" data-name="${items[item].nom_client_blue}" href="#">Voir Fiche</a>
                                    </div>
                                    `;
                            ...
                            // Création d'une instance de bounds
                            ...
                            //     position
                            // }) => {
                            //     console.log("Bounding the maps")
                            //     // console.log(marker, position)
                            //     // console.log(position)
                            //     if (parseFloat(position.gps_latitude_control) && parseFloat(position.gps_longitude_control)) {
                            //         bounds.extend({
                            //             lat: parseFloat(position.gps_latitude_control),
                            //             lng: parseFloat(position.gps_longitude_control),
                            //         });
                            //     }
                            // });
