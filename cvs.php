                        ...
                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                            <div class="page-header">
                                <div id="breadcrumbs" class="clearfix">
                                    <a href="cvs.php" class="breadcrumbs_home"><i class="fab fa-fw fa-wpforms nav_icon"></i>  <?php echo $mnu_title; ?></a> <span class="raquo">»</span></div>
                            </div>
                        </div>
                    </div> 
                    <div class="row"> 
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <table class="table table-bordered table-hover">
                                        ...
                                        <tbody>
<?php
if($utilisateur->HasDroits("10_519")){ 	 
$num_line = 0;
while ($row_ = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    echo "<td>" . $item->GetOrganismeList($row_["code"]). "</td>";
    $province->code = $row_["code_province"];
    //$spinn =$province->GetDetailIN();
    $province->GetDetailIN();
    //echo '<td>'.$spinn["libelle"]."</td>";
    echo '<td>' . $province->libelle . "</td>";
    $commune->code = $row_["id_commune"];
    $spinn = $commune->GetDetail();
    echo '<td>' . $spinn["libelle"] . "</td>";
// echo "<td>".$row_["activated"]."</td>";
    echo '<td><button type="button" data-toggle="dropdown" class="btn btn-secondary dropdown-toggle" aria-expanded="false"></button>
                    ...
                    <div class="modal-body">
                        <form id="mainForm" method="post" action="controller.php" enctype="multipart/form-data">  
						 ...
						 <div class="form-group">
								 <label>Site</label> 
								  <select class='form-control select2' style='width: 100%;' id='id_site' name='id_site' required >
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group"  style="width: 100%;" > 
                                    <select  class="form-control select2" name="code_province" id="code_province" style="width: 100%;" required>
                                        <option selected='selected' disabled >Veuillez choisir</option>
                                        <?php
                                        $stmt_select = $province->read('4');
                            ...
                            <div class="form-group">
                                <label>Commune</label>
                                <div class="input-group"  style="width: 100%;" > 
                                    <select  class="form-control select2" name="id_commune" id="id_commune" style="width: 100%;" required>
                                        <!--  <option selected='selected' disabled >Veuillez choisir</option>  -->
                                        <?php
                                        /* $stmt_select = $commune->read();
                                          while ($row_select = $stmt_select->fetch(PDO::FETCH_ASSOC)) {
                                          echo "<option value=".$row_select["code"].">{$row_select["libelle"]}</option>";
                                          } */
                                        ?>
                            ...
                            <div class="form-group">
                                <label>CVS</label>
                                <div class="input-group"  style="width: 100%;" > 
                                    <input type="text" class="form-control pull-right" name="libelle" id="libelle" required>
                                </div> 
                            </div> 
							 <div class="form-group">
                            <label>Organisme *</label>			
                            <select class='form-control select2' style='width: 100%;' id='id_organisme' name='id_organisme[]' required   multiple="multiple">
<?php
...
?></select>
                            ...
                            <!--  <div class="form-group">
                                      <label>Activé</label>
                                      <div class="input-group"  style="width: 100%;" > 
                                        <input type="text" class="form-control pull-right" name="activated" id="activated">
                                      </div> 
                              </div> -->
        ...
        <script>
            ...
            var actual_commune="";
                $("#dlg_main .select2").each(function() {
				 ...
				 $('#id_organisme').select2({ tags: true });
...
<?php //if ($utilisateur->HasDroits("12_40")) {
    ?>	 $(".delete").click(function(e) {	
                        ...
                        swal({
                            ...
                            if (isConfirm) {
                                var view_mode = "delete_cvs";
                                $.ajax({
                                    url: "controller.php",
                                    method: "POST",
                                    data: {view: view_mode, code: jeton_actuel},
...
<?php //if($utilisateur->HasDroits("12_38")){   ?>
                $("#btn_new_").click(function() {
                    ClearForm();
                    $("#dlg_main #view").val("create_cvs");
                    $("#titre").html("CREATION CVS");
                    $("#dlg_main").modal("show");
                });
<?php //}  ?>
                function ClearForm() {
...
<?php //if($utilisateur->HasDroits("12_39")){   ?>
                $(".edit").click(function(e) {
                    ...
                    ClearForm();
                    var jeton_actuel = jQuery(this).attr("data-id");
                    $("#titre").html("MODIFICATION INFORMATIONS CVS");
                    $.ajax({
                        url: "controller.php",
                        method: "GET",
                        dataType: "json",
                        data: {view: "detail_cvs", code: jeton_actuel},
                        success: function(result, statut) {
                            $("#dlg_main #view").val("edit_cvs");
                            $("#id_site").val(result.data.id_site).change();
                            $("#code").val(result.data.code);
                            $("#code_province").val(result.data.code_province).change();
                            //$("#id_organisme").val(result.data.id_organisme).change();
							var selectedItems = [];
							 $.each(result.items, function(i, item) {
								 selectedItems.push(item.code);
								//$("#id_organisme option[value='" + item.code + "']").prop('selected', true);
															
									});
								 $("#id_organisme").val(selectedItems).change();	
                            actual_commune = result.data.id_commune;
                            $("#libelle").val(result.data.libelle);
                            //$("#activated").val(result.data.activated);
                            $("#dlg_main").modal("show");
                        },
                        error: function(resultat, statut, erreur) {
                        }
