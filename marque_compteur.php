<?php 
 // session_start();
 $mnu_title = "Marques des compteurs";
 $page_title = "Liste des Marques des compteurs";
 $home_page = "index.php";
 $page_root = "marque_compteur.php";
 $active = "marque_compteur";
 $parambase = " active";
...
Autoloader::Load('classes');
 ...
 include_once "classes/class.marque_compteur.php";*/
...
$from_record_num = ($records_per_page * $page) - $records_per_page;
 ...
 <button type="submit" name="search" id="search-btn" class="btn btn-primary"><i class="fa fa-search"></i>
 						...
 						<table class="table table-bordered table-hover">
 							<thead>
 								<tr> 
 <th style="width:5%;"></th>
 <th>Marque compteur</th>
                                             </tr>
                                         </thead>
                                        <tbody>
         <?php
 				if($utilisateur->HasDroits("10_525"))
 				{ 	 
 								$num_line=0;
 ...
 <?php if($utilisateur->HasDroits("10_460"))
 	{  ?>	 $(".delete").click(function () {
 					...
 					swal({
 										...
 										if (isConfirm) {     
 											   var view_mode ="delete_marque_compteur";
 											  $.ajax({
 												   url:"controller.php",
 												   method:"POST",
 ...
 <?php if($utilisateur->HasDroits("10_460"))
 	{  ?>	 $("#btn_new_").click(function () {	
 					 ClearForm();
 					 $("#dlg_main #view").val("create_marque_compteur");
 					 $("#dlg_main #Heading").html("CREATION MARQUE COMPEUR");
 					  $("#dlg_main").modal("show");
 	}); <?php } ?>
   function ClearForm(){
 	$("#dlg_main #view").val("");
 ...
 <?php if($utilisateur->HasDroits("10_460"))
 	...
 	$(".edit").click(function () {	
 				 ClearForm();
 				var jeton_actuel = jQuery(this).attr("data-id");
 				$("#dlg_main #Heading").html("MODIFICATION INFORMATIONS MARQUE");
 				  $.ajax({
 					   url:"controller.php",
 					   method:"GET",
 					   dataType:"json",
 					   data: {view:"detail_marque_compteur",code:jeton_actuel},
 					   success : function(result, statut){  
 							$("#dlg_main #view").val("edit_marque_compteur");
