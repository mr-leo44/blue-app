<?php 
 ...
 // session_start();
 $mnu_title = "Unité de mesure";
 $page_title = "Liste des Unités de mesure";
 $home_page = "index.php";
 $page_root = "unite_de_mesure.php";
 $active = "unite_de_mesure";
 $parambase = " active";
...
Autoloader::Load('classes');
 ...
 include_once "classes/class.unite_de_mesure.php";*/
...
$from_record_num = ($records_per_page * $page) - $records_per_page;
 ...
 <div class="container-fluid  dashboard-content"> 
 	<div class="row">
 		...
 		<a href="<?php echo $page_root;?>" class="breadcrumbs_home"><i class="fab fa-fw fa-wpforms nav_icon"></i>  <?php echo $mnu_title; ?></a> <span class="raquo">»</span></div>
                        </div>
                    </div>
                </div>
 	   <div class="row"> 
 ...
 <th>Unité de mesure</th>
                                        ...
                                        <tbody>
         <?php
 				if($utilisateur->HasDroits("10_522"))
 				{ 	 
 								$num_line=0;
 							...
 							if($utilisateur->HasDroits("10_460"))
 								{		
 							echo '<a href="#" class="dropdown-item edit"  data-id="'.$row_["code_unite"].'">Modifier</a>';
 								}	
 					if($utilisateur->HasDroits("10_460"))
 {		echo '<a href="#" class="dropdown-item delete" data-name="'.$row_["libelle_unite"].'" data-id="'.$row_["code_unite"].'">Supprimer</a>';
 						}
 						?>
 					</ul>
 										</td>
	<?php echo "<td>".$row_["libelle_unite"]."</td>";?>
 									</tr>
 							<?php			}	
 				}			?>   	
 							</tbody>
 ...
 <div class="modal fade" id="dlg_main" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
   ...
   <form id="mainForm" method="post" action="controller.php" enctype="multipart/form-data">  
 		   <input name="code_unite"  id="code_unite" type="hidden"> 
 		   <input name="view" id="view" type="hidden"> 
         <div class="form-group">
                 <label>Unité de mesure</label>
                 <div class="input-group"  style="width: 100%;" > 
                   <input type="text" class="form-control pull-right" name="libelle_unite" id="libelle_unite">
                 </div> 
         </div> 
 ...
 <?php if($utilisateur->HasDroits("10_460"))
 	{  ?>	 $(".delete").click(function () {
 					...
 					swal({
 										...
 										if (isConfirm) {     
 											   var view_mode ="delete_unite_de_mesure";
 											  $.ajax({
 												   url:"controller.php",
 												   method:"POST",
 									  data:{view:view_mode,code_unite:jeton_actuel},
 ...
 <?php if($utilisateur->HasDroits("10_460"))
 	{  ?>	 $("#btn_new_").click(function () {	
 					 ClearForm();
 					 $("#dlg_main #view").val("create_unite_de_mesure");
 					 $("#dlg_main #Heading").html("CREATION UNITE_DE_MESURE");
 					  $("#dlg_main").modal("show");
 	}); <?php } ?>
   function ClearForm(){
 	$("#dlg_main #view").val("");
 ...
 <?php if($utilisateur->HasDroits("10_460"))
 	...
 	$(".edit").click(function () {	
 				 ClearForm();
 				var jeton_actuel = jQuery(this).attr("data-id");
 				$("#dlg_main #Heading").html("MODIFICATION INFORMATIONS UNITE_DE_MESURE");
 				  $.ajax({
 					   url:"controller.php",
 					   method:"GET",
 					   dataType:"json",
 					   data: {view:"detail_unite_de_mesure",code_unite:jeton_actuel},
 					   success : function(result, statut){  
 							$("#dlg_main #view").val("edit_unite_de_mesure");
 							$("#libelle_unite").val(result.data.libelle_unite);
 $("#code_unite").val(result.data.code_unite);
 $("#libelle_unite").val(result.data.libelle_unite);
 							$("#dlg_main").modal("show");
 					   },
 					   error : function(resultat, statut, erreur){
