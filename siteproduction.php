...
// session_start();
$mnu_title = "Site de Production";
$page_title = "Liste des Sites de Production";
$home_page = "index.php";
					...
					<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
						<div class="page-header">
							...
							<div id="breadcrumbs" class="clearfix">
								<a href="<?php echo 'siteproduction.php'; ?>" class="breadcrumbs_home"><i class='fab fa-fw fa-wpforms'></i> <?php echo $mnu_title; ?></a> <span class="raquo">»</span>
							</div>
						</div>
					</div>
				</div>
						...
						<div class="card">
							<div class="card-body">
								...
								<table class="table table-bordered table-hover">
									...
									<tbody>
										<?php
										if ($utilisateur->HasDroits("10_517")) {
											$num_line = 0;
											while ($row_ = $stmt->fetch(PDO::FETCH_ASSOC)) {
												$num_line++;
												echo "<tr><td>" . $num_line . "</td>";
												echo "<td>" . $row_["intitule_site"] . "</td>";
												$province->code = $row_["province_id"];
												$spinn = $province->GetDetail();
												echo '<td>' . $spinn["libelle"] . "</td>";
												echo "<td>" . $row_["adresse_site"] . "</td>";
												echo "<td>" . $row_["contact_site"] . "</td>";
												echo '<td>
 								...
 								<ul class="dropdown-menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(705px, 40px, 0px); top: 0px; left: 0px; will-change: transform;">';
												...
												echo '<a href="#" class="dropdown-item edit"  data-id="' . $row_["code_site"] . '">Modifier</a>';
												//	}	
												//if($utilisateur->HasDroits("12_40")){		
												echo '<a href="#" class="dropdown-item delete" data-name="' . $row_["intitule_site"] . '" data-id="' . $row_["code_site"] . '">Supprimer</a>';
												//}
										?>
												</ul>
												</td>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="mainForm" method="post" action="controller.php" enctype="multipart/form-data">
						<input name="code_site" id="code_site" type="hidden">
						<input name="view" id="view" type="hidden">
						<div class="form-group">
							<label>Site</label>
							<div class="input-group" style="width: 100%;">
								<input type="text" class="form-control pull-right" name="intitule_site" id="intitule_site">
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<label>Adresse</label>
							<div class="input-group" style="width: 100%;">
								<input type="text" class="form-control pull-right" name="adresse_site" id="adresse_site">
							</div>
						</div>
						<div class="form-group">
							<label>Contact</label>
							<div class="input-group" style="width: 100%;">
								<input type="text" class="form-control pull-right" name="contact_site" id="contact_site">
							</div>
						</div>
		...
		$(function() {
			...
			$(".delete").click(function() {
				...
				swal({
					...
					if (isConfirm) {
						var view_mode = "delete_siteproduction";
						$.ajax({
							url: "controller.php",
							method: "POST",
							data: {
								view: view_mode,
								code_site: jeton_actuel
							},
			...
			$("#btn_new_").click(function() {
				ClearForm();
				$("#dlg_main #view").val("create_siteproduction");
				$("#dlg_main #Heading").html("CREATION SITE DE PRODUCTION");
				$("#dlg_main").modal("show");
			});
			<?php //} 
			?>
			...
			$(".edit").click(function() {
				ClearForm();
				var jeton_actuel = jQuery(this).attr("data-id");
				$("#dlg_main #Heading").html("MODIFICATION INFORMATIONS SITE DE PRODUCTION");
				$.ajax({
					url: "controller.php",
					method: "GET",
					dataType: "json",
					data: {
						view: "detail_siteproduction",
						code_site: jeton_actuel
					},
					success: function(result, statut) {
						$("#dlg_main #view").val("edit_siteproduction");
						$("#code_site").val(result.data.code_site);
						$("#code_site").val(result.data.code_site);
						$("#intitule_site").val(result.data.intitule_site);
						$("#code_province").val(result.data.province_id).change();
						$("#adresse_site").val(result.data.adresse_site);
						$("#contact_site").val(result.data.contact_site);
						$("#dlg_main").modal("show");
					},
					error: function(resultat, statut, erreur) {}
				});
