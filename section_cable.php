<?php 
 ...
 // session_start();
 $mnu_title = "Section Câble";
 $page_title = "Liste des sections câbles";
 $home_page = "index.php";
 $active = "section_cable";
 $parambase = " active";
...
jQuery(document).delegate('a.delete-item', 'click', function(e) {
		...
		var label=$('tr.item-row[data-id="' + itemId + '"]').find('span.sn').html();     
 					...
 					swal({
 										...
 										if (isConfirm) {     
 											   var view_mode ="delete_section_cable";
 											  $.ajax({
 												   url:"controller.php",
 												   method:"POST",
 					   dataType:"json",
 ...
 <?php //if($utilisateur->HasDroits("12_38")){  ?>	 $("#add_line").click(function () {	
					...
					ClearR();
					 $("#dlg_frm #view").val("create_section_cable");
 ...
 jQuery(document).delegate('a.edit-item', 'click', function(e) {
		...
		var itemId = $(this).parents('tr.item-row').attr('data-id'); 
		var label=$('tr.item-row[data-id="' + itemId + '"]').find('span.sn').html();    
		$("#dlg_main #view").val("edit_section_cable");
